...
public class MobileIOSDriver extends BaseMobileDriver implements MobileAppDriver {
	...
	private void initialize() {
		...
		// Optional params
		this.appiumPort = xmlParameters.get(appiumPort) == null ? Constants.APPIUM.DEFAULT_PARAMS.APPIUM_PORT
				: xmlParameters.get(appiumPort);
		this.fullReset = xmlParameters.get(fullReset) == null ? Constants.APPIUM.OPTIONAL_PARAMS.IOS.GENERAL.FULL_RESET
				: xmlParameters.get(fullReset);
		this.clearSystemFiles = xmlParameters.get(clearSystemFiles) == null
				? Constants.APPIUM.OPTIONAL_PARAMS.IOS.GENERAL.CLEAR_SYSTEM_FILES
				: xmlParameters.get(clearSystemFiles);
	}
	...
	/**
	 * <a href=
	 * "https://appium.io/docs/en/writing-running-appium/caps/index.html">...</a>
	...
	private DesiredCapabilities getDesiredCapabilities() {
		...
		// Default params
		capabilities.setCapability("automationName", Constants.APPIUM.DEFAULT_PARAMS.IOS.GENERAL.AUTOMATION_NAME);
		capabilities.setCapability("resetOnSessionStartOnly",
				Constants.APPIUM.DEFAULT_PARAMS.IOS.GENERAL.RESET_ON_SESSION_START_ONLY);
		// Optional params
	...
	/**
	 * Singleton The same instance of AppiumDriver will be returned no matter how
	 * many times it is called. This ensures that only one instance of the driver
	 * exists at any time.
	 */
	@Override
